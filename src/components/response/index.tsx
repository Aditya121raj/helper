...
import noviewLogo from "../../../assets/icons/noview_logo.svg";
...
type InputMode = "screenshot" | VoiceSource | "history";
type VoiceStatus = "idle" | "listening" | "finalizing" | "thinking" | "complete";

export interface ResponseProps {
  setView: (view: "initial" | "response" | "followup") => void;
}

const RESPONSE_WIDTH = 1050;
...
export default function Response({ setView }: ResponseProps) {
  ...
  const initialVoiceSessionId = window.sessionStorage.getItem("noview.voiceSessionId");
  const initialVoiceSource = window.sessionStorage.getItem("noview.voiceSource") as VoiceSource | null;
  ...
  const [inputMode, setInputMode] = useState<InputMode>(
    initialVoiceSource || "screenshot",
  );
  const [screenshotStatus, setScreenshotStatus] = useState<"analyzing" | "complete">("analyzing");
  const [voiceTranscript, setVoiceTranscript] = useState("");
  const [voiceSource, setVoiceSource] = useState<VoiceSource | null>(initialVoiceSource);
  const [voiceNotice, setVoiceNotice] = useState("");
  const [voiceElapsedSeconds, setVoiceElapsedSeconds] = useState(0);
  const [llmUsage, setLlmUsage] = useState<LlmUsageSnapshot | null>(null);
  const [voiceStatus, setVoiceStatus] = useState<VoiceStatus>(
    initialVoiceSessionId ? "listening" : "idle",
  );
  ...
  useEffect(() => {
    window.electronAPI.getLlmUsage?.()
      .then((response) => setLlmUsage(response?.success ? response.data ?? null : null))
      .catch(() => undefined);
    const cleanupFunctions = [
      window.electronAPI.onLlmUsageUpdated?.((usage) => setLlmUsage(usage)),
      ...
      window.electronAPI.onResponseStart(() => {
        const activeVoiceSession = window.sessionStorage.getItem(
          "noview.voiceSessionId",
        );
      ...
      window.electronAPI.onResponseSuccess((rawData: any) => {
        const completedVoiceSource = window.sessionStorage.getItem(
          "noview.voiceSource",
        ) as VoiceSource | null;
        window.sessionStorage.removeItem("noview.voiceSessionId");
        window.sessionStorage.removeItem("noview.voiceSource");
        if (completedVoiceSource) {
          setInputMode(completedVoiceSource);
          setVoiceSource(completedVoiceSource);
        ...
        const finalResponse =
          responseText || (currentStreaming && currentStreamed ? currentStreamed : "");
        queryClient.setQueryData(["task_response"], { response: finalResponse, isFollowUp });
        updateResponseData(finalResponse);
      }),
      window.electronAPI.onVoiceRecordingStarted(({ sessionId, source }) => {
      ...
      window.electronAPI.onVoiceIncrementalTranscript(({ transcript, sessionId, source }) => {
        if (
          window.sessionStorage.getItem("noview.voiceSessionId") !== sessionId
        ) return;
        setInputMode(source);
      ...
      window.electronAPI.onVoiceFinalizing(({ source }) => {
        setInputMode(source);
        setVoiceSource(source);
        setVoiceStatus("finalizing");
      }),
      window.electronAPI.onVoiceTranscriptFinalized(({ transcript, source }) => {
        setInputMode(source);
        setVoiceTranscript(transcript);
        setVoiceSource(source);
        setVoiceStatus("thinking");
      }),
      window.electronAPI.onVoiceConflict((message) => {
        setVoiceNotice(message);
        window.setTimeout(() => setVoiceNotice(""), 2500);
      }),
      window.electronAPI.onVoiceError((error) => {
        const failedVoiceSource = window.sessionStorage.getItem(
          "noview.voiceSource",
        ) as VoiceSource | null;
        window.sessionStorage.removeItem("noview.voiceSessionId");
        window.sessionStorage.removeItem("noview.voiceSource");
        if (failedVoiceSource) {
          setInputMode(failedVoiceSource);
          setVoiceSource(failedVoiceSource);
          setVoiceStatus("complete");
        }
  ...
  const isVoiceInput =
    inputMode === "system-audio" || inputMode === "microphone";
  ...
  const voiceElapsedTime = `${String(Math.floor(voiceElapsedSeconds / 60)).padStart(2, "0")}:${String(voiceElapsedSeconds % 60).padStart(2, "0")}`;
          ...
          >
            ...
            <div className="flex justify-between items-center px-4 py-3 flex-shrink-0"
                ...
                <span className="text-sm font-medium whitespace-nowrap text-white" style={{ color: 'rgba(255, 255, 255, 0.9)' }}>
                  {errorMessage
                    ...
                    ? voiceStatus === "listening"
                      ? voiceSource === "system-audio" ? "Listening to System Audio..." : "Listening to Microphone..."
                      : voiceStatus === "finalizing"
                      ? voiceSource === "system-audio"
                        ? "Transcribing System Audio..."
                        : "Transcribing Microphone..."
                      : voiceStatus === "thinking"
                        ? "Generating answer..."
                        : "AI Response"
                    : inputMode === "screenshot" && screenshotStatus === "analyzing"
              ...
              <div className="flex items-center gap-2 flex-shrink-0">
                  {llmUsage && (
                    <span
                      className="text-[11px] font-medium px-2 py-1 rounded-full text-white/70 border border-white/15 bg-white/5"
                      title={`${llmUsage.inputTokens} input + ${llmUsage.outputTokens} output tokens${llmUsage.estimated ? ' (estimated)' : ''}`}
                    >
                      {llmUsage.provider} · {llmUsage.contextRemaining != null && llmUsage.contextLimit
                        ? `${Math.max(0, Math.min(100, Math.round((llmUsage.contextRemaining / llmUsage.contextLimit) * 100)))}% left`
                        : llmUsage.rateLimitRemainingTokens != null
                        ? `${llmUsage.rateLimitRemainingTokens.toLocaleString()} rate tokens left`
                        : `${llmUsage.totalTokens.toLocaleString()} tokens`}
                      {llmUsage.fallbackAttempts > 0 ? ` · fallback ${llmUsage.fallbackAttempts}` : ''}
                    </span>
                  )}
                  <span 
            ...
            <div 
                ...
                {inputMode === "screenshot" ? (
                  <>
                    ...
                    {screenshotStatus === "analyzing" && (
                      <p className="mt-2 text-sm text-white/65">Analyzing...</p>
                    )}
                  </>
                ) : inputMode === "history" ? (
                  <h2 className="text-sm font-semibold text-white">
                  ...
                  <>
                    ...
                    {voiceTranscript && voiceStatus !== "finalizing" && (
              ...
              <section
                data-section="final-ai-response"
                className="mt-3 rounded-2xl border border-white/10 bg-white/[0.03] px-4 py-4"
              >
                  ...
                  <div className="flex flex-col gap-4 py-3">
                    <div>
                      ...
                      <p className="mt-2 text-sm text-white/70 max-w-2xl">
                        {isRateLimitError
                          ? "Gemini returned a 429 (Too Many Requests). Please wait a few seconds and try again."
