...
const PROVIDER_OPTIONS: Array<{ id: LlmProviderId; name: string; keyPlaceholder: string }> = [
  ...
  { id: "openrouter", name: "OpenRouter", keyPlaceholder: "sk-or-..." },
  { id: "groq", name: "Groq", keyPlaceholder: "gsk_..." },
  { id: "custom", name: "Custom OpenAI-compatible", keyPlaceholder: "Optional API key" },
];
...
export default function Tooltip({ trigger, onVisibilityChange }: TooltipProps) {
  ...
  const loadCurrentConfig = useCallback(async () => {
    try {
      ...
      if (response?.success && response.data) {
        ...
        const provider = configData.primaryProvider || "gemini";
        const providerConfig = configData.providers.find((item) => item.provider === provider);
        setSelectedProvider(provider);
        setSelectedModel(providerConfig?.model || providerConfig?.models?.[0]?.id || "");
      ...
      const timeout = setTimeout(() => {
        const checkUpdate = async () => {
          try {
            ...
            releaseUrl: result.data.releaseUrl || 'https://github.com/Aditya121raj/helper/releases'
                });
  ...
  useEffect(() => {
    ...
    const downloadUrl = url || updateInfo?.releaseUrl || 'https://github.com/Aditya121raj/helper/releases';
      try {
  ...
  const handleSaveConfig = useCallback(async () => {
    ...
    try {
      ...
      if (response?.success) {
        ...
        setError(null);
        if (response.data) setLlmConfig(response.data);
        setApiKey("");
  ...
  }, [apiKey, selectedProvider, selectedModel, baseUrl, loadCurrentConfig]);

  const selectedProviderConfig = llmConfig?.providers.find((item) => item.provider === selectedProvider);
  const availableModels = selectedProviderConfig?.models || [];
  const selectedProviderMeta = PROVIDER_OPTIONS.find((item) => item.id === selectedProvider) || PROVIDER_OPTIONS[0];

  const handleProviderChange = useCallback((provider: LlmProviderId) => {
    setSelectedProvider(provider);
    setApiKey("");
    setError(null);
    const providerConfig = llmConfig?.providers.find((item) => item.provider === provider);
    setSelectedModel(providerConfig?.model || providerConfig?.models?.[0]?.id || "");
    setBaseUrl(providerConfig?.baseUrl || "");
  }, [llmConfig]);
      ...
      >
        ...
        >
          <div className="space-y-4 px-4 py-3">
            ...
            <div className="mb-4">
              <a 
                href="https://github.com/Aditya121raj/helper"
                target="_blank" 
                ...
                onClick={async (e) => {
                  ...
                  try {
                    ...
                    if (window.electronAPI?.openUpdateDownload) {
                      await window.electronAPI.openUpdateDownload('https://github.com/Aditya121raj/helper');
                    } else {
                      // Fallback to default link behavior
                      window.open('https://github.com/Aditya121raj/helper', '_blank', 'noopener,noreferrer');
                    }
                  } catch (error) {
              ...
              <div className="mb-4">
                ...
                >
                  <div className="flex items-start gap-2">
                    ...
                    <span className="underline font-semibold text-sm">github.com/Aditya121raj/helper/releases</span>
                      </div>
                      <button
                        onClick={async () => {
                          if (!isInteractive) return;
                    const downloadUrl = updateInfo.releaseUrl || 'https://github.com/Aditya121raj/helper/releases';
                          try {
                            await window.electronAPI?.openUpdateDownload?.(downloadUrl);
                          } catch (error) {
                            console.error("Error opening update download:", error);
            ...
            <div className="mb-4">
              ...
              <div className="space-y-3">
                <div>
                  ...
                  <select
                    ...
                    className={`w-full px-4 py-2 rounded-lg text-white text-sm transition-all duration-200 ${isTransparent ? '' : 'bg-white/10 border border-white/20'} ${
                      isInteractive ? 'focus:outline-none focus:ring-2 focus:ring-blue-500/20' : 'cursor-default'
                    }`}
                    style={isTransparent ? { background: 'transparent', border: 'none' } : {}}
                  >
                ...
                <div className="flex items-center justify-between rounded-lg px-3 py-2 bg-white/5 border border-white/10">
                  <span className="text-xs text-white/65">Connection</span>
                  <span className={`text-xs font-medium ${
                    selectedProviderConfig?.connectionState === 'connected'
                      ? 'text-emerald-300'
                      : selectedProviderConfig?.connectionState === 'checking'
                      ? 'text-blue-300'
                      : selectedProviderConfig?.connectionState === 'rate_limited'
                      ? 'text-amber-300'
                      : selectedProviderConfig?.connectionState === 'invalid' || selectedProviderConfig?.connectionState === 'error'
                      ? 'text-red-300'
                      : 'text-white/50'
                  }`}>
                    {selectedProviderConfig?.connectionState === 'connected'
                      ? `Connected · ${selectedProviderConfig.models.length} models`
                      : selectedProviderConfig?.connectionMessage || 'Not tested'}
                  </span>
                </div>

                <div>
                  <label className="block text-xs text-white/70 mb-1 text-center">{selectedProviderMeta.name} API Key</label>
                  <input
                    ref={apiKeyInputRef}
                    type="password"
                    value={apiKey}
                    ...
                    disabled={!isInteractive}
                    tabIndex={isInteractive ? 0 : -1}
                    placeholder={selectedProviderConfig?.maskedApiKey || selectedProviderMeta.keyPlaceholder}
                    ...
                    style={isTransparent ? { background: 'transparent', border: 'none' } : {}}
                  />
                  {selectedProviderConfig?.hasApiKey && (
                    <p className="mt-1 text-[11px] text-white/45 text-center">Blank chhodoge to saved encrypted key use hogi.</p>
                  )}
                </div>

                {selectedProvider === 'custom' && (
                  <div>
                    <label className="block text-xs text-white/70 mb-1 text-center">OpenAI-compatible Base URL</label>
                    <input
                      type="text"
                      value={baseUrl}
                      onChange={(event) => isInteractive && setBaseUrl(event.target.value)}
                      disabled={!isInteractive}
                      tabIndex={isInteractive ? 0 : -1}
                      placeholder="http://127.0.0.1:11434/v1"
                      className={`w-full px-4 py-2 rounded-lg text-white text-sm placeholder-white/40 transition-all duration-200 ${isTransparent ? '' : 'bg-white/10 border border-white/20'} ${
                        isInteractive ? 'focus:outline-none focus:ring-2 focus:ring-blue-500/20' : 'cursor-default'
                      }`}
                      style={isTransparent ? { background: 'transparent', border: 'none' } : {}}
                    />
                ...
                <div>
                  ...
                  >
                    {!availableModels.length && selectedModel && (
                      <option value={selectedModel} className="bg-gray-800 text-white">{selectedModel}</option>
                    )}
                    {!availableModels.length && !selectedModel && (
                      <option value="" className="bg-gray-800 text-white">Test connection to load models</option>
                    )}
                ...
                {llmConfig && (
                  <div className="rounded-lg px-3 py-2 bg-white/5 border border-white/10">
                    <div className="text-[11px] text-white/45 mb-1">Fallback chain</div>
                    <div className="text-xs text-white/75 truncate">
                      {[llmConfig.primaryProvider, ...llmConfig.fallbackOrder]
                        .filter((provider, index, list) => list.indexOf(provider) === index)
                        .filter((provider) => llmConfig.providers.find((item) => item.provider === provider)?.enabled)
                        .map((provider) => PROVIDER_OPTIONS.find((item) => item.id === provider)?.name || provider)
                        .join(' → ') || 'Connect a provider first'}
                    </div>
                  </div>
                )}
              </div>
