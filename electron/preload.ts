...
interface ElectronAPI {
  ...
  setLlmRouting: (data: { primaryProvider: LlmProviderId; fallbackOrder: LlmProviderId[] }) => Promise<{ success: boolean; data?: any; error?: string }>;
  getLlmUsage: () => Promise<{ success: boolean; data?: any; error?: string }>;
  onLlmUsageUpdated: (callback: (usage: any) => void) => () => void;
...
const electronAPI = {
  ...
  setApiConfig: (config: { provider: LlmProviderId; apiKey?: string; model?: string; baseUrl?: string; makePrimary?: boolean }) =>
    ipcRenderer.invoke("set-api-config", config),
  getApiConfig: () => ipcRenderer.invoke("get-api-config"),
  setLlmRouting: (data: { primaryProvider: LlmProviderId; fallbackOrder: LlmProviderId[] }) =>
    ipcRenderer.invoke("set-llm-routing", data),
  getLlmUsage: () => ipcRenderer.invoke("get-llm-usage"),
  onLlmUsageUpdated: (callback: (usage: any) => void) => {
    const subscription = (_event: unknown, usage: any) => callback(usage);
    ipcRenderer.on("llm-usage-updated", subscription);
    return () => ipcRenderer.removeListener("llm-usage-updated", subscription);
  },
  ...
  voiceCaptureReady: (sessionId: string, source: VoiceSource) =>
    ipcRenderer.invoke("voice-capture-ready", { sessionId, source }),
  voiceAudioChunk: (sessionId: string, source: VoiceSource, pcm: string, sampleRate: number) =>
    ipcRenderer.invoke("voice-audio-chunk", { sessionId, source, pcm, sampleRate }),
  finalizeVoiceRecording: (sessionId: string, source: VoiceSource) =>
    ipcRenderer.invoke("finalize-voice-recording", { sessionId, source }),
  cancelVoiceRecording: (sessionId?: string) => ipcRenderer.invoke("cancel-voice-recording", { sessionId }),
  reportVoiceRecordingError: (sessionId: string, message: string) =>
    ipcRenderer.invoke("voice-recording-error", { sessionId, message }),
  voicePerformanceEvent: (event: string, sessionId?: string, fields?: Record<string, string | number | boolean | null>) =>
    ipcRenderer.send("voice-performance-event", { event, sessionId, fields }),
  onVoiceRecordingStarted: (callback: (data: { sessionId: string; source: VoiceSource }) => void) => {
    const subscription = (_event: unknown, data: { sessionId: string; source: VoiceSource }) => callback(data);
  ...
  onVoiceRecordingStopRequested: (callback: (data: { sessionId: string; source: VoiceSource }) => void) => {
    const subscription = (_event: unknown, data: { sessionId: string; source: VoiceSource }) => callback(data);
    ipcRenderer.on("voice-recording-stop-requested", subscription);
    return () => ipcRenderer.removeListener("voice-recording-stop-requested", subscription);
  },
  onVoiceIncrementalTranscript: (callback: (data: { transcript: string; sessionId: string; source: VoiceSource }) => void) => {
    const subscription = (_event: unknown, data: { transcript: string; sessionId: string; source: VoiceSource }) => callback(data);
    ipcRenderer.on("voice-incremental-transcript", subscription);
    return () => ipcRenderer.removeListener("voice-incremental-transcript", subscription);
  },
  onVoiceTranscriptFinalized: (callback: (data: { transcript: string; sessionId: string; source: VoiceSource }) => void) => {
    const subscription = (_event: unknown, data: { transcript: string; sessionId: string; source: VoiceSource }) => callback(data);
    ipcRenderer.on("voice-transcript-finalized", subscription);
    return () => ipcRenderer.removeListener("voice-transcript-finalized", subscription);
  },
  onVoiceFinalizing: (callback: (data: { sessionId: string; source: VoiceSource }) => void) => {
    const subscription = (_event: unknown, data: { sessionId: string; source: VoiceSource }) => callback(data);
    ipcRenderer.on("voice-finalizing", subscription);
    return () => ipcRenderer.removeListener("voice-finalizing", subscription);
  },
  ...
  onVoiceConflict: (callback: (message: string) => void) => {
    const subscription = (_event: unknown, message: string) => callback(message);
    ipcRenderer.on("voice-conflict", subscription);
    return () => ipcRenderer.removeListener("voice-conflict", subscription);
  },
} as ElectronAPI;
