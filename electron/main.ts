import { BrowserWindow, app, screen, Menu, desktopCapturer } from "electron";
import { ProcessingHelper } from "./ProcessingHelper";
import { ScreenCaptureHelper } from "./ScreenCaptureHelper";
import { ScreenshotHelper } from "./ScreenshotHelper";
import { ShortcutsHelper } from "./shortcuts";
import { VoiceTranscriptionHelper } from "./VoiceTranscriptionHelper";
import { logVoicePerformance } from "./VoicePerformanceLogger";
import { VoiceSessionStateMachine, VoiceSource } from "./VoiceSessionStateMachine";
...
async function initializeStore() {
  ...
  const configPath = path.join(userDataPath, "noview", "config.json");
...
export async function deleteStoreValue(key: string): Promise<boolean> {
  if (!store) {
    const initialized = await initializeStore();
    if (!initialized || !store) return false;
  }
  // The lightweight JSON store exposes atomic set(); replacing the legacy
  // secret with null removes the sensitive value without bypassing its lock.
...
interface State {
  ...
  voiceTranscriptionHelper: VoiceTranscriptionHelper | null;
  voiceSessionController: VoiceSessionStateMachine;
  ...
  history: string[];
  historyIndex: number;
  screenshotOperationActive: boolean;
}
...
const state: State = {
  ...
  voiceTranscriptionHelper: null,
  voiceSessionController: new VoiceSessionStateMachine(),
  llmManager: null,
  view: "initial",
  step: 0,
  ...
  history: [],
  historyIndex: -1,
  screenshotOperationActive: false,
  PROCESSING_EVENTS: {
...
export interface IShortcutsHelperDeps {
  ...
  runScreenshotOperation: () => Promise<void>;
  ...
  toggleVoiceRecording: (source: VoiceSource) => void;
}
...
export interface initializeIpcHandlerDeps {
  ...
  voiceTranscriptionHelper: VoiceTranscriptionHelper | null;
  voiceSessionController: VoiceSessionStateMachine;
  processVoiceTranscript: (transcript: string, source: VoiceSource, sessionId?: string) => Promise<string>;
  saveResponseToHistory: (responseText: string) => void;
    ...
    try {
      ...
      if (typeof width === "string" && width === "fixed") {
        ...
        if (state.view === "initial") {
          ...
          // Keep response/followup aligned with the enlarged command bar.
          if (lockedResponseWidth === null) {
            lockedResponseWidth = 1050;
...
function initializeHelpers() {
  ...
  state.voiceTranscriptionHelper = new VoiceTranscriptionHelper(
    (sessionId, transcript) => {
      const current = state.voiceSessionController.snapshot();
      ...
      window.webContents.send("voice-incremental-transcript", {
        sessionId,
    ...
    state.voiceTranscriptionHelper.prewarm(process.env.NOVIEW_VOICE_MODEL === "tiny" ? "tiny" : process.env.NOVIEW_VOICE_MODEL === "base" ? "base" : "small");
  ...
  state.processingHelper = new ProcessingHelper({
    ...
    getLlmManager: () => {
      if (!state.llmManager) throw new Error("LLM manager is not initialized.");
      return state.llmManager;
    },
...
function isAnyOperationActive(): boolean {
  return Boolean(
    state.screenshotOperationActive ||
    state.voiceSessionController.snapshot() ||
    state.processingHelper?.isProcessing(),
  );
}

function showOperationBusyNotice(): void {
  const mainWindow = getMainWindow();
...
async function runScreenshotOperation(): Promise<void> {
  if (isAnyOperationActive()) {
    showOperationBusyNotice();
    return;
  }

  state.screenshotOperationActive = true;
  const mainWindow = getMainWindow();

  try {
    if (mainWindow && !mainWindow.isDestroyed()) {
      mainWindow.webContents.send(state.PROCESSING_EVENTS.INITIAL_START);
    }

    await takeScreenshot();

    if (!state.processingHelper) {
      throw new Error("Processing helper is not ready.");
    }
    ...
    if (message.includes("Screenshot capture already in progress")) {
      showOperationBusyNotice();
    } else {
      console.error("Screenshot operation failed:", error);
      if (mainWindow && !mainWindow.isDestroyed()) {
        mainWindow.webContents.send(
          state.PROCESSING_EVENTS.INITIAL_RESPONSE_ERROR,
          message,
        );
      }
    }
  } finally {
    state.screenshotOperationActive = false;
  }
}

function toggleVoiceRecording(source: VoiceSource): void {
  const mainWindow = getMainWindow();
  if (!mainWindow || mainWindow.isDestroyed() || !state.voiceTranscriptionHelper) return;

  const activeVoiceSession = state.voiceSessionController.snapshot();

  if (activeVoiceSession) {
    if (activeVoiceSession.source !== source) {
      showOperationBusyNotice();
      return;
    }

    const activeResult = state.voiceSessionController.toggle(source);
    if (activeResult.action === "stop") {
      mainWindow.webContents.send("voice-recording-stop-requested", {
        sessionId: activeResult.session.id,
        source: activeResult.session.source,
      });
    } else if (activeResult.action === "ignored") {
      showOperationBusyNotice();
    }
    return;
  }
  ...
  if (source === "system-audio" && process.platform !== "win32") {
    mainWindow.webContents.send("voice-error", "System Audio capture is available only on Windows.");
    return;
  }

  const result = state.voiceSessionController.toggle(source);
  if (result.action === "conflict") {
    mainWindow.webContents.send("voice-conflict", result.message);
    return;
  }
  ...
  const voiceModel = process.env.NOVIEW_VOICE_MODEL === "tiny" ? "tiny" : process.env.NOVIEW_VOICE_MODEL === "base" ? "base" : "small";
  state.voiceTranscriptionHelper.start(voiceModel, result.session.id).then((sessionId) => {
    const current = state.voiceSessionController.snapshot();
    if (!current || current.id !== sessionId) return;
    setView("response");
    mainWindow.webContents.send("voice-recording-started", { sessionId, source });
  }).catch((error: Error) => {
    state.voiceSessionController.finish(result.session.id);
    mainWindow.webContents.send("voice-error", error.message);
  });
}
...
function createWindow(): BrowserWindow {
  ...
  state.step = 60;
  state.currentY = 0;
  const initialWindowWidth = 1050;
  const initialWindowHeight = 120;
  state.currentX = (workArea.width - initialWindowWidth) / 2;
  ...
  state.mainWindow.webContents.session.setPermissionRequestHandler((_webContents, permission, callback) => {
    callback(permission === "media" || permission === "display-capture");
  });
  state.mainWindow.webContents.session.setPermissionCheckHandler(
    (_webContents, permission) => permission === "media",
  );
  state.mainWindow.webContents.session.setDisplayMediaRequestHandler((_request, callback) => {
    ...
    desktopCapturer.getSources({ types: ["screen"], thumbnailSize: { width: 0, height: 0 } })
      .then((sources) => {
        ...
        const video = sources.find((source) => source.display_id === primaryDisplayId) || sources[0];
        if (!video) {
          callback({});
          return;
        }
        // Electron's Windows loopback stream is backed by Chromium's WASAPI capture.
        callback({ video, audio: "loopback" });
      })
      .catch((error) => {
        console.error("Unable to initialize Windows WASAPI loopback:", error);
        callback({});
      });
  });
...
async function initializeApp() {
  try {
    ...
    state.llmManager = new LlmManager();
    await state.llmManager.initialize();
    const legacyApiKey = await getStoreValue("api-key");
    const storedLegacyModel = await getStoreValue("api-model");
    const legacyModel = !storedLegacyModel || /^gemini-2\.(?:0|5)-flash(?:$|-)/i.test(storedLegacyModel)
      ? "gemini-3.6-flash"
      : storedLegacyModel;
    if (legacyApiKey) {
      await state.llmManager.migrateLegacyGemini(legacyApiKey, legacyModel);
      if (state.llmManager.getPublicConfig().providers.find((provider) => provider.provider === "gemini")?.hasApiKey) {
        await deleteStoreValue("api-key");
        console.log("Legacy Gemini key migrated to the encrypted LLM vault.");
      }
    }
    state.llmManager.refreshConnections().catch((error) => {
      console.warn("Background LLM connection refresh failed:", error);
    });
    ...
    initializeIpcHandlers({
      ...
      voiceTranscriptionHelper: state.voiceTranscriptionHelper,
      voiceSessionController: state.voiceSessionController,
      processVoiceTranscript: async (transcript, source, sessionId) => {
        if (!state.processingHelper) throw new Error("Processing helper is not ready.");
        return state.processingHelper.processVoiceTranscript(transcript, source, sessionId);
      },
      saveResponseToHistory,
      llmManager: state.llmManager,
    });
...
async function getConfiguredModel(): Promise<string> {
  try {
    const model = await getStoreValue("api-model");
    return !model || /^gemini-2\.(?:0|5)-flash(?:$|-)/i.test(model)
      ? "gemini-3.6-flash"
      : model;
  } catch (error) {
...
if (!hasSingleInstanceLock) {
  console.log("Another NoView instance is already running. Closing this instance.");
  app.quit();
} else {
...
app.on("before-quit", () => {
  ...
  cleanupAllFiles();
  state.voiceTranscriptionHelper?.shutdown();
