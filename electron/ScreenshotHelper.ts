...
export class ScreenshotHelper {
  ...
  private async captureScreenshotWindows(): Promise<Buffer> {
    ...
    try {
      ...
      const physicalWidth = Math.max(1, Math.round(primaryDisplay.size.width * scaleFactor));
      const physicalHeight = Math.max(1, Math.round(primaryDisplay.size.height * scaleFactor));
      const resizeScale = Math.min(1, 1600 / physicalWidth);
      const thumbnailSize = {
        width: Math.max(1, Math.round(physicalWidth * resizeScale)),
        height: Math.max(1, Math.round(physicalHeight * resizeScale)),
      };
      ...
      if (!source || source.thumbnail.isEmpty()) {
        throw new Error("Electron desktop capture returned an empty image");
      }
      const buffer = source.thumbnail.toPNG();
      console.log(`[ScreenshotPerf] Windows native capture completed in ${Math.round(performance.now() - startedAt)}ms`);
      return buffer;
    } catch (nativeError) {
      console.warn("Native Windows screenshot capture failed; using PowerShell fallback:", nativeError);
    }
    ...
    return this.safeFileOperation(async () => {
      ...
      const buffer = await fs.promises.readFile(tmpPath);
      await fs.promises.unlink(tmpPath);
      console.log(`[ScreenshotPerf] PowerShell fallback completed in ${Math.round(performance.now() - fallbackStartedAt)}ms`);
      return buffer;
  ...
  public async takeScreenshot(): Promise<string> {
    ...
    try {
      ...
      console.log(`[ScreenshotPerf] Screenshot ready in ${Math.round(performance.now() - startedAt)}ms`);
