...
class UpdateService {
      ...
      if (hasUpdate) {
        ...
        return {
          ...
          releaseInfo: {
            ...
            releaseUrl: latestRelease.html_url || githubConfig.releasesUrl,
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (response.status === 404) {
          console.log('[UpdateService] No published GitHub release found yet');
          return null;
        }
  ...
  public async openReleasesPage(url?: string): Promise<void> {
    const releasesUrl = url || this.getGitHubConfig().releasesUrl;
